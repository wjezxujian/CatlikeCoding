...
public class StableFloatRigidbody : MonoBehaviour
{
    [SerializeField]
    ...
    void FixedUpdate()
    {
        ...
        for (int i = 0; i < buoyancyOffsets.Length; ++i)
        {
            if (submergence[i] > 0f)
            {
                float drag = Mathf.Max(0f, 1 - dragFactor * submergence[i]);
                body.velocity *= drag;
                body.angularVelocity *= drag;
                body.AddForceAtPosition(gravity * -(buoyancy * submergence[i]), transform.TransformPoint(buoyancyOffsets[i]), ForceMode.Acceleration);
